...
IEK_Filter::IEK_Filter(const ros::NodeHandle &nh,
                                 const ros::NodeHandle &pnh)
    : nh_(nh), pnh_(pnh) {
  infoSub_ = nh_.subscribe("noisy_pose_topic", 1, &IEK_Filter::odom_Callback, this);
  posePub_ = nh_.advertise<nav_msgs::Odometry>("est_pose_topic", 1000);
  double t_0 = 0;
  double dt = 0;
}
void IEK_Filter::init(){
...
void IEK_Filter::odom_Callback(const nav_msgs::Odometry &msg) {
...
void IEK_Filter::Iterated_Extended_Kalman_Filter(){
    xo = x;
...
void IEK_Filter::state_prediction(){
       xp   << (x(0) + cos(x(2))*u(0)*dt),
               (x(1) + sin(x(2))*u(0)*dt),
               (x(2) + u(1)*dt);
       xp(2) = std::fmod(xp(2) + M_PI, 2.0 * M_PI) - M_PI;
}
void IEK_Filter::covariance_prediction(){
       Pp = F * P * F.transpose() + Qp;
}
void IEK_Filter::kalman_gain(){
       K = Pp * G.transpose() * (G * Pp * G.transpose() + Rp).inverse();
}
void IEK_Filter::state_correction(){
...
void IEK_Filter::state_correction_I(){
...
void IEK_Filter::covariance_correction(){
       P = (I - K * G) * Pp;
}
void IEK_Filter::covariance_correction_I(){
       P = (I - K * G) * Pp;
}
void IEK_Filter::motion_model_Jacobian(){
   F   << 1, 0, (-sin(x(2))*u(0)*dt),
          0, 1, (cos(x(2))*u(0)*dt),
          0, 0, 1;
}
void IEK_Filter::motion_noise_Jacobian(){
   wp   <<  cos(x(2)), 0,
            sin(x(2)), 0,
            0,         1;
   Qp = dt*dt * wp * Q * wp.transpose();
}
void IEK_Filter::observation_model_Jacobian(){
   G   << (x(0))/(sqrt((x(0)*x(0))+(x(1))*(x(1)))), (x(1))/(sqrt((x(0)*x(0))+(x(1))*(x(1)))), 0,
          (-x(1))/(sqrt((x(0)*x(0))+(x(1))*(x(1)))), (x(0))/(sqrt((x(0)*x(0))+(x(1))*(x(1)))), -1;
}
void IEK_Filter::observation_model_Jacobian_O(){
...
void IEK_Filter::observation_noise_Jacobian(){
   Rp = R;
}
void IEK_Filter::observation_noise_Jacobian_O(){
   Rp = R;
}
}
// namespace Kalman
